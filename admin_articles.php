...
$S = GetVar('S');
$msg = GetVar('msg');
$Reviewed_Since = GetVar('Reviewed_Since');
$Reviewed_Before = GetVar('Reviewed_Before');
$NoteType = GetVar('NoteType');
$ReadStats = GetVar('ReadStats');
$GroupID = GetVar('GroupID');
$ViewableBy = GetVar('ViewableBy');
$STATUS = GetVar('STATUS');
$Priority = GetVar('Priority');
$MustRead = GetVar('MustRead');
$Custom1 = GetVar('Custom1');
$Custom2_Before = GetVar('Custom2_Before');
$Custom2_Since = GetVar('Custom2_Since');
$Keywords = GetVar('Keywords');
$ReviewBy = GetVar('ReviewBy');
$Created_Since = GetVar('Created_Since');
$Created_Before = GetVar('Created_Before');
$Modified_Since = GetVar('Modified_Since');
$Modified_Before = GetVar('Modified_Before');
$CREATEDBY = GetVar('CREATEDBY');
$AuthorOrContact = GetVar('AuthorOrContact');
$Title = GetVar('Title');
$MyGroups = GetVar('MyGroups');
$SType = GetVar('SType');
...
?>
    ...
    <body>
        ...
        if (GetVar('S')) {
            ...
            {
                if (strlen((string)$str) > 250) $str = substr((string)$str, 0, 250) . "...";
            ...
            $DBFields["Hits"] = " ";
            if (trim((string)$Reviewed_Since) || trim((string)$Reviewed_Before)) {
                $DBFields["LastReviewed:Reviewed"] = "@fmt_dateonly:nowrap";
            } else {
                if (trim((string)$NoteType))
            ...
            if (trim((string)$Product)) {
                $q .= " and Product like " . $AppDB->qstr("%" . trim((string)$Product) . "%");
            ...
            if (trim((string)$Created_Since)) {
                $q .= " and Articles.CREATED >= '$Created_Since'";
                if (!trim((string)$Created_Before))
            ...
            if (trim((string)$Modified_Since)) {
                ...
                if (!trim((string)$Modified_Before))
            ...
            if (trim((string)$Contact1)) {
                $Contact1 = trim((string)$Contact1);
                $q .= " and (Contact1 like '%$Contact1%' OR Contact2 like '%$Contact1%') ";
                $qtxt .= "$and Contact is like $Contact1 ";
            }

            if (trim((string)$AuthorOrContact)) {
                $AuthorOrContact = trim((string)$AuthorOrContact);
                        ...
                        <table <? echo $FORM_STYLE ?> width="550">
                            ...
                            <tr>
                                ...
                                <td class="form-data"><? DBField("Articles", "Product_S", $Product);
                                                        PopupFieldValues("Articles", "form", "Product"); ?> </td>
                            </tr>
                            <tr>
