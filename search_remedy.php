...
function BuildRemedySearchQuery($Search,$QSub,$SMethod,$SType='')
{
	...
	$HelpDeskTable = "T" . $AppDB->Settings->HelpDeskTable;
	...
	// We only do this on the HelpDesk form not the workInfo table as the WorkInfo table has only one column we
	// search so we do not need the join per word search on that table.
	if ($NewMethod) {
		$i = 1;
		foreach($Words as $Word) {
			$HPDRankT .= " $plus KT$i" . ".Rank ";
			$plus = "+";
    	  	$QHPD2 .= "\n\t\tinner join $SMethod(".$HelpDeskTable.", $FTColumn,'$Word'," . MAXROWS . ") as KT$i ON HD.Entry_ID=KT$i.[KEY] ";
			++$i;
			$SPhrase = $and . "'$Word'";
			$and = " AND ";
		} 
		...
		$HPDRankT = " KT1.Rank ";
        $QHPD2 .= "\n\t\tinner join $SMethod(" . $HelpDeskTable.", $FTColumn,'$Search'," . MAXROWS . ") as KT1 ON  HD.Entry_ID=KT1.[KEY] ";
	}
	...
	select  HPD.Incident_Number as ID,
			HPD.Entry_ID as Entry_ID,
	 		HPD.Incident_Number as Incident_Number,
			HPD.Description as Summary,
			HPD.Reported_Date as Date,
			HPD.Assigned_Group as Group_,
			HPD.Product_Name as Product,
			HPD.Detailed_Decription as Notes,
			RANK as Rank
	from
	(
		select Merged.Incident_Number,sum(RANK) as RANK 
			from (
	...
	$query .= '						
			union all
				select WL.Incident_Number,
					'. $WLRankT .' as RANK
					from HPD_WorkLog as WL	
					'. $QWL2 .'
		';
	}
	$query .= '
			) 
			as Merged group by Incident_Number

	) as Results

	inner join HPD_Help_Desk as HPD on Results.Incident_Number = HPD.Incident_Number
	...
	if ($What == "Remedy Known Error") {
		header("location:search_remedy_ke.php?S=Search&Search=$Search&SType=$SType&Advanced=$Advanced");
		exit;
	}
...
<table width="98%" border="0" align="center" cellpadding="4">
  <tr>
    <td colspan="2">
  <tr valign="middle">
    <td width="23%" align="left" valign="top" nowrap><p class="subhdr">Search Remedy Incidents:</p>
    <span style="font-size:9px">(Search most remedy fields including Work Info)</span></td>
    <td width="77%"><p>
      <form name="searchform" method="get" action="<? echo $PHP_SELF ?>">
        ...
        <input name="S" type="submit" value="Search"> 	
			...
			<? } else { ?>
				<a class="small" href="search_remedy.php?What=<? echo $What ?>&SType=<? echo $SType ?>&Advanced=1" title="Advanced Search filters">Advanced</a>
			<? } ?>
      </form>
    </td>
  </tr>
...
function fmt_Summary($Title,$ID,$R) 
{
	...
	$Entry_ID = $R['Entry_ID'];
		...
		if ($Previews) {
			$t =  "<p class=RPreview><a title=\"click to view in Remedy\" href=\"OpenTicket.php?Server=" . 
				$AppDB->Settings->RemedyARServer  . "&ID=$Entry_ID\"><img align=\"absmiddle\" src=\"images/aruser_sm.gif\" border=\"0\"></a> ";
				if (defined("REMEDY_SHOW_CASE_URL")) {
					$t .= '<a target=_blank title="click to view details" href="'.REMEDY_SHOW_CASE_URL.'&Server=' . $AppDB->Settings->RemedyARServer . "&ID=$ID\">" . $Title . "</a></p>\n";	
				} else
					$t .= $Title . "</p>";	
				$t .= "<p class=RPreview>" . fmt_Preview($R[Notes]) . "</p>"; //<hr>";
		}
		else {
			$t =  "<a title=\"click to view in Remedy\" href=\"OpenTicket.php?Server=" . 
				$AppDB->Settings->RemedyARServer  . "&ID=$Entry_ID\"><img align=\"absmiddle\" src=\"images/aruser_sm.gif\" border=\"0\"></a> ";		
				if (defined("REMEDY_SHOW_CASE_URL")) {
					$t .= '<a target=_blank title="click to view details" href="'.REMEDY_SHOW_CASE_URL.'&Server=' . $AppDB->Settings->RemedyARServer . "&ID=$ID\">" . $Title . "</a></p>\n";	
				} else
...
function fmt_product($p)
{
	...
	if (!$ns && ($Search || $Group || $Type || $Product)) { 
		...
		$DBFields["Incident_Number:Incident"] = " ";
		...
		if ($Search) {
			...
			HPD.Product_Name as Product,
			HPD.Detailed_Decription as Notes
			 from HPD_Help_Desk as HPD where 1=1 ";
		}
		if (trim($Group)) {
