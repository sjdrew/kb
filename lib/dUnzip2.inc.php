...
#				$this->debugMsg(1, "Reached end of file");
			...
			elseif($signature == $this->dirSignatureE){
				...
				$this->debugMsg(1, "No valid header, ending loop at offset ".ftell($fh));
				break;
			}
		}
	...
	Function unzip($compressedFileName, $targetFileName=false){
		...
		if(!$fdetails['uncompressed_size']){ // file is not compressed and empty
			$this->debugMsg(1, "File '<b>$compressedFileName</b>' is empty.");
			if ($targetFileName) { 
				$byteswritten = file_put_contents($targetFileName, "");
			}
			else $byteswritten  = 0;
			touch($targetFileName,$fdetails['lastmod_datetime']);
			return $byteswritten;
		}
		
		fseek($this->fh, $fdetails['contents-startOffset']);
		$contents  = $this->uncompress(
