...
function CopyCategory(KB)
{
	if (FindElement('Categorization')) {
		
		var KB,CatType,CatT1,CatT2,CatT3,CatProd,CatOrg,CatGroup;
		
		if (FindElement('CatType')) CatType = escape(FindElement('CatType').innerHTML);
		if (FindElement('CatT1')) CatT1 = escape(FindElement('CatT1').innerHTML);
		if (FindElement('CatT2')) CatT2 = escape(FindElement('CatT2').innerHTML);
		if (FindElement('CatT3')) CatT3 = escape(FindElement('CatT3').innerHTML);
		if (FindElement('CatProd')) CatProd = escape(FindElement('CatProd').innerHTML);
		if (FindElement('CatOrg')) CatOrg = escape(FindElement('CatOrg').innerHTML);
		if (FindElement('CatGroup')) CatGroup = escape(FindElement('CatGroup').innerHTML);
		var d = new Date();
		var t = d.getTime();
		var params = "?KB="+KB+"&CatType="+CatType+"&CatT1="+CatT1+"&CatT2="+CatT2+"&CatT3="+CatT3+"&CatProd="+CatProd+"&CatOrg="+CatOrg+"&CatGroup="+CatGroup+"&t="+t;	
		alert(params);
		var oXML = new XMLobj() ;
		if (oXML.LoadUrl('lib/StoreCategorization.php' + params) == "OK") {
			alert("Categorization Copied. Now click 'KB Paste' button on Remedy Incident form");
		} else {
			alert("Unable to Copy Categorization.");	
		}
	}
}
