...
var MTMnoCollapseSubFolders = true; // sjd
...
function MTMDisplayItem(item, last) {
	...
	if(item.submenu) {
		...
		if (MTMnoCollapseSubFolders) {

		}
		else {
		 	MTMClickCmd = "return " + MTMfrm + ".MTMSubAction(" + MTMfrm + MTMref + ");";
			MTMouseOverCmd = "parent.status='" + (item.expanded ? "Collapse " : "Expand ") + (item.text.indexOf("'") != -1 ? MTMEscapeQuotes(item.text) : item.text) + "';return true;";
			MTMouseOutCmd = "parent.status=parent.defaultStatus;return true;";
		}
