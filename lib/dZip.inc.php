...
class dZip{
	...
	Function addFile($filename, $cfilename, $fileComments='', $data=false){
		if(!($fh = &$this->fh)) {
			...
			if (!$fh) {
				$this->Error = "unable to create zip file $this->filename. " . "cwd = " . getcwd();
				return false;
			}
		}
		...
		$lastmod_timeS  = str_pad(decbin(date('s',$mtime)>=32?date('s',$mtime)-32:date('s',$mtime)), 5, '0', STR_PAD_LEFT);
		$lastmod_timeM  = str_pad(decbin(date('i',$mtime)), 6, '0', STR_PAD_LEFT);
		$lastmod_timeH  = str_pad(decbin(date('H',$mtime)), 5, '0', STR_PAD_LEFT);
		$lastmod_dateD  = str_pad(decbin(date('d',$mtime)), 5, '0', STR_PAD_LEFT);
		$lastmod_dateM  = str_pad(decbin(date('m',$mtime)), 4, '0', STR_PAD_LEFT);
		$lastmod_dateY  = str_pad(decbin(date('Y',$mtime)-1980), 7, '0', STR_PAD_LEFT);
