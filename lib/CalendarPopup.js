...
function CalendarPopup_select(inputobj, linkname, format) {
	...
	if (inputobj.value!="") {
		var specs = inputobj.value.split(/ /);
		var time_str;
		var date_str;
		date_str = specs[0];
		time_str = (specs[1]) ? specs[1] : "";
		var time = getDateFromFormat(date_str,format);
		if (time==0) { 
...
function CalendarPopup_getCalendar() {
	...
	if (this.displayType=="date" || this.displayType=="week-end") {
		...
		var selected_time = "";
		if (this.currentTime) {
			selected_time = " " + this.currentTime;
		}
		...
		for (var row=1; row<=6; row++) {
			...
			for (var col=1; col<=7; col++) {
				...
				if (this.disabledWeekDays[col-1]) {
					...
					result += '	<TD CLASS="'+td_class+'"><A HREF="javascript:'+windowref+this.returnFunction+'('+selected_year+',\''+selected_month+'\',\''+selected_date+'\',\''+selected_time+'\');'+windowref+'CalendarPopup_hideCalendar(\''+this.index+'\');" CLASS="'+date_class+'">'+display_date+'</A></TD>\n';
					}
				display_date++;
		...
		if (this.disabledWeekDays[current_weekday+1]) {
			...
			selected_date = now.getDate();
			selected_month = (now.getMonth())+1;
			selected_year = now.getFullYear();
			if (selected_date < 10) selected_date = "" + "0" + selected_date;
			if (selected_month < 10) selected_month = "" + "0" + selected_month;		
			result += '		<A CLASS="textlink" HREF="javascript:'+windowref+this.returnFunction+'('+selected_year+',\''+selected_month+'\',\''+selected_date+'\',\''+selected_time+'\');'+windowref+'CalendarPopup_hideCalendar(\''+this.index+'\');">'+this.todayText+'</A>\n';
			}
		result += '		<BR>\n';
		//if (this.showTime) {
		//	result += '<form name="tform"><input type="text" size="6" name="TIME" value="' + this.currentTime + '"></form>';
		//}
