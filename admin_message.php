		...
		if (!$Err) {
			...
			if ($SendEmail) {
				...
				if (count($MailTo)) {
					...
					$Vars['Author'] = $Author;
					$Vars['TicketNumber'] = $TicketNumber;
					$Vars['StartTime'] = $StartTime;
					$Vars['EndTime'] = $EndTime;
					$Vars['Escalated'] = $Escalated;
					$Vars['Message'] = nl2br($Message);
					...
					if ($HtmlMsg) {
						...
						$Num = send_mail(array(),"Bulletin Notification: $Subject",$HtmlMsg,$TextMsg,$from,$MailTo);					
				...
				if ($ID) {
					...
					if ($_POST["StartTime"] == "") $_POST["StartTime"] = NowDateTime();
...
Bulletin Board Message<br><br></span></td>
...
<? 	hidden("ID",$ID);
	...
	if (!$ID) {
		...
		$Grps = split(",",$CUser->u->GroupIDs);
		$M->GroupID = $Grps[0];
		$M->StartTime = NowDateTime();
	}	
	
	 ?>    
<table width="560" cellspacing="8" cellpadding="0">
  <tr>
  <td> 
        <table width="100%" <? echo $FORM_STYLE ?> >
            <tr>
                <td width="25%" align="right" CLASS="form-hdr">
                  Date                </td>
                <td CLASS="form-data">
            ...
            <tr>
                <td width="25%" CLASS="form-hdr" align="right">
                  From                </td>
                <td width="75%" CLASS="form-data">
            ...
            <tr>
              ...
              <td CLASS="form-data"><?	DBField($Table,"ServiceName",$M->ServiceName);?> <font size="1">(if applicable)</font> </td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td CLASS="form-data"><?	DBField($Table,"StartTime",$M->StartTime);?> <font size="1"> (if MUI provide date/time that problem first noticed)</font> </td>
            </tr>
            <tr>
