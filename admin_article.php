...
function ProcessSave($ID,$rdonly,&$msg,&$Err,$ModifyAll=0)
{
	...
	if ($AppDB->Settings->PrivMode == "Group" && $ID) {
		...
		if 	($ID && $CUser->u->GroupArray[$OldRec->GroupID] == "W" && !$AppDB->Settings->AllowModifyArticles && 
	...
	if ($_POST[delete_attachment]) {
		$DAR = $AppDB->GetRecordFromQuery("select ID,Filename from ArticleAttachments where ID=$_POST[delete_attachment]");
		if ($DAR) {
			// Save everything before we delete this 
			$ArchiveID = CreateArchiveRecord($OldRec);
			$AppDB->sql("delete from ArticleAttachments where ID=$_POST[delete_attachment]");
	...
	if ($_POST[RemoveAID]) {
		if (DeleteArticleVersion($_POST[RemoveAID]))
			$msg .= "Version $_POST[RemoveAID] deleted.";
		return $ID;
	}

	if ($_POST[RestoreAID]) {
		if (RestoreArticleVersion($ID,$OldRec,$_POST[RestoreAID]))
			$msg .= "Version $_POST[RestoreAID] Restored as Current.";
		return $ID;
	}
	
	if ($AppDB->Settings->PrivMode == "Group") {
		$OldGroupID = $OldRec->GroupID;
		
		// check that this User has A or W priv in new Group
		if ($OldGroupID != $_POST[GroupID]) {
			if (
				$CUser->u->Priv != PRIV_ADMIN &&
		...
		if ($_POST[STATUS] == "Active" && $OldRec->STATUS != "Active")  {
				if ($_POST[LastReviewed] == "" && $CUser->u->Priv != PRIV_ADMIN) {
					$msg = "Article $ID must be Reviewed by pressing the 'Mark Reviewed' button, before being set Active.";
					$Err = 1;
					$_POST[STATUS] = $OldRec->STATUS;
					return $ID;				
				}
				if ($CUser->u->Priv != PRIV_ADMIN && $CUser->u->GroupArray[$_POST[GroupID]] != "A") {		
		...
		//
		if (!$Err) {
			if ($_POST[ViewableBy] == PRIV_GUEST && $OldRec->ViewableBy != PRIV_GUEST &&
				$_POST[STATUS] == "Active" &&
				$CUser->u->Priv != PRIV_ADMIN /* && 
				$CUser->u->GroupArray[$_POST[GroupID]] != "A" */) {
		...
		if (is_array($ModFields) && count($ModFields) > 0) {
			...
			foreach($ModFields as $MField => $OldValue) {
				...
				if ($MField == "Content") {
					...
					$OGroup = $AppDB->GetRecordFromQuery("select Name from Groups where GroupID = $OldValue");
					$NGroup = $AppDB->GetRecordFromQuery("select Name from Groups where GroupID = " . $_POST[GroupID]);
	...
	if ($ID) {
		...
		if ($_POST[STATUS] == "Pending Technical Review" && $OldRec->STATUS != "Pending Technical Review") {
			$msg .= SendNotifications($ID,"NotifyTechnicalReview");
		} 
		if ($_POST[STATUS] == "Pending Content Review" && $OldRec->STATUS != "Pending Content Review") {
...
function ProcessDelete($ID,&$msg,$Multi=0)
{
	...
	else if ($Reviewed == "Mark Reviewed" && $_POST['STATUS'] == "Pending Technical Review") {
		$_POST['STATUS'] = "Pending Content Review";
		$Save = "Save";
	...
	else if ($Reviewed && $_POST['STATUS'] == "Pending Content Review") {
		$_POST[STATUS] = "Active";
	...
	else if ($Reviewed && $_POST['STATUS'] == "Active") {		
		if ($AppDB->Settings->DefReviewPeriod) {
			$_POST[ReviewBy] = MonthAdd('',$AppDB->Settings->DefReviewPeriod); 		
			$msg = "Article has been marked reviewed and the next review date set to " . $_POST[ReviewBy];	
		}
		$Save = "Save";
		$_POST[LastReviewed] = $Now;
		$_POST[LastReviewedBy] = $CUser->UserID;
	...
	if ($AddRelated && $AddRelatedID) {
		if (strtoupper(substr($AddRelatedID,0,2)) == "KB") {
			$AddRelatedID = (int) substr($AddRelatedID,2);
		}
		if ($AddRelatedID > 0) {
	...
	if ($RemoveRelatedID) {
		$AppDB->sql("delete from Related where ID = $RemoveRelatedID");
		$msg = "Relationship removed.";
	}

	if ($_POST[delete_attachment] || $_POST[RemoveAID] || $_POST[RestoreAID]) {
		$Save = "Save"; 
	}	
	...
	if ($Delete && $_POST[ID] && !$IDLIST) {
		if (ProcessDelete($_POST[ID],$msg)) {
	...
	if ($ID) {
		...
		if ($AppDB->Settings->PrivMode == "Group") {
			...
			if 	($ID && $CUser->u->Priv != PRIV_ADMIN && $CUser->u->GroupArray[$F['GroupID']] == "W" && !$AppDB->Settings->AllowModifyArticles && 
					$F['STATUS'] == "Active") {
		...
		$Content = str_replace("src=\"files/KB0","src=\"/".DBNAME."/files/KB0",$Content);
		...
		$AttachmentAsContent = $AttR->ID;
		
	}
	
	if ($_POST && !$_POST["RestoreAID"]) {
		// keep reposted values, but strip slashes
		repost_stripslashes();
		if ($CopyToNew) {
			$ID = $LASTMODIFIEDBY = $LASTMODIFIED = $CREATED = $CREATEDBY = $LastReviewed = $LastReviewedBy = "";
			$STATUS="Compose";
			$ReviewBy ="";
		...
		if ($AppDB->Settings->PrivMode != "Simple") {
			foreach($CUser->u->GroupArray as $GID => $GMode) {
				if ($GID != 1) { $GroupID = $GID; break; }
			}
			$ViewableBy = PRIV_GROUP;
		}
		else $ViewableBy = PRIV_SUPPORT;
	}
...
?>
<center>
<form onSubmit="return ParseForm();" name=form action="<? echo $PHP_SELF ?>" method="post">
  ...
  <table width="100%" border=0 cellspacing=0 cellpadding=0>
    <tr>
      <td width="25%" class="subhdr"><img align="middle" src="images/newticket.jpg" width="51" height="51"><span style="">
...
<!--  <div style="float:middle"> !-->
 ...
 <? 
    ...
    <table width="95%" cellpadding="0" height="450px" cellspacing="0" class="tabtable">
      <tr>
        <td valign="top" align="center" width="100%"><!--<div class="shadowboxfloat">
                ...
                <tr>
				  ...
				  <table width="100%" >
				    <tr>
			          ...
			          <td rowspan="3" class="form-hdr"><table cellpadding="2">
                        <tr>
...
Hits</a>
				    ...
				    <tr>
                      ...
                      <td class="form-data"><? DBField("Articles","Title",$Title,$ModifyAll ? 2 : 0); ?></td>
               	    </tr>
                <? if ($AppDB->Settings->PrivMode == "Group") { ?>
                <tr>
                ...
                <? } else hidden("GroupID","0"); ?>
				  ...
				  <td valign="top" style="padding:5px" width="50%">
				    <fieldset style="padding: 5px; height:100%;"><legend>Classification</legend>
				  <table width="100%">
                <tr>
                  <td class="form-hdr">Viewable By:</td>
                  <td class="form-data"><? 
				  	if ($AppDB->Settings->PrivMode == "Group") {
						DBField("Articles","ViewableByG",$ViewableBy,0,$ModifyAll);  
					}
					else {
						DBField("Articles","ViewableBy",$ViewableBy,0,$ModifyAll);  					
					}	
...
&nbsp;</td>
                ...
                <tr>
                  ...
                  <td class="form-data"><? DBField("Articles","Type",$Type); ?>
&nbsp;</td>
                </tr>
                <tr>
                  ...
                  <td class="form-data"><? 
						...
						if ($RemDB) {						
		 				  	dropdownlistfromquery("Product",$RemDB,
									"select Product_Name FROM PCT_ProductCompanyAssocLookup
										where Status_PDC = 1 and Status_PDA = 1 AND Help_Desk_Selection = 0
										order by Product_Name",
									$Product,
									$Product,
									'style="width:175px"',"Product_Name","Product_Name"); 
						} else {
					  		DBField("Articles","Product",$Product); PopupFieldValues("Articles","form","Product"); 
						}					
						?>
&nbsp;</td>
                </tr>
                <tr>
                  ...
                  <td class="form-data"><? DBField("Articles","MustRead",$MustRead,($CUser->u->Priv != PRIV_ADMIN),$ModifyAll); ?></td>
                </tr>
				<? if ($AppDB->Settings->Custom1Label) { ?>
                <tr>
                  <td nowrap valign="top" class="form-hdr"><? echo $AppDB->Settings->Custom1Label ?></td>
                  <td class="form-data"><? DBField("Articles","Custom1",$Custom1,0,$ModifyAll); ?></td>
                </tr>
				<? } ?>
				<? if ($AppDB->Settings->Custom2Label) { ?>
                <tr>
                  <td nowrap valign="top" class="form-hdr"><? echo $AppDB->Settings->Custom2Label ?></td>
                  <td class="form-data"><? DBField("Articles","Custom2",$Custom2,0,$ModifyAll); ?></td>
                </tr>
				<? } ?>
                <tr>
                  <td nowrap valign="top" class="form-hdr"><p>Search Keywords:<br>
                      <span class="small"><em>(Specify same keyword more than</em></span></p>
                    <p class="small"><em> once to increase ranking).  </em></p>                    </td>
                  <td class="form-data"><? DBField("Articles","Keywords",$Keywords,0,$ModifyAll); ?></td>
                </tr>
                <tr>
                  ...
                  <td class="form-data"><input <? if (!$ID) echo "Disabled" ?> type="submit" name="AddRelated" value="Add">
				  			<input type="text" size="8" maxlength="8" name="AddRelatedID">				  </td>
				</tr>
				<tr>
				<td style="padding:3px" colspan="2">
				  ...
				  if ($ID) {
					...
					while($RRec = $AppDB->sql_fetch_obj($RRes)) {
						...
						echo "<tr><td><p class=\"form-sm\"><a title=\"Remove relationship\" href=\"Javascript:RemoveRelated($RRec->ID)\"><img border=0 src=\"images/delete.gif\"></a></p></td>" .
							 "<td><p class=\"form-sm\"><a target=_blank href=\"article.php?ID=$RRec->IDB\" title=\"Click to view in new window\">$RRec->IDB</a></p></td><td><p class=\"form-sm\">" . substr($RRec->Title,0,54) . "...</p></td></tr>\n";
					}
					if (!$first) echo "</table>";
				  }
				  ?>				 </td>
				  ...
				  <td style="padding:5px" valign="top" height="250" width="50%"><fieldset style="padding: 5px; height:100%;" ><legend>Status</legend>
				  <table width="100%">
                <tr>
                  <td class="form-hdr">Status:</td>
                  <td class="form-data"><? DBField("Articles","STATUS",$STATUS,0,$ModifyAll); ?>                  </td>
                </tr>
                <tr>
                  <td class="form-hdr">Created:</td>
                  <td class="form-data"><? echo $CREATED; if ($CREATEDBY) echo " ($CREATEDBY)"; ?>&nbsp; </td>
                </tr>
                <tr>
                  <td class="form-hdr">Modified:</td>
                  <td class="form-data"><? echo $LASTMODIFIED; if ($LASTMODIFIED) echo " ($LASTMODIFIEDBY)"; ?>&nbsp;</td>
                </tr>
                <tr>
                  <td nowrap class="form-hdr">Content Modified: </td>
                  <td class="form-data"><? echo "$ContentLastModified"; ?></td>
                </tr>
                <tr>
                  <td class="form-hdr">1st Contact:</td>
                  <td class="form-data"><? DBField("Articles","Contact1",$Contact1); ?><? //PopupFieldValues("Articles","form","Contact1"); ?></td>
                </tr>
                <tr>
                  <td class="form-hdr">2nd Contact:</td>
                  <td class="form-data"><? DBField("Articles","Contact2",$Contact2); ?><? //PopupFieldValues("Articles","form","Contact2"); ?></td>
                </tr>
                <tr>
                  <td class="form-hdr">Review By:</td>
                  <td class="form-data"><? DBField("Articles","ReviewBy",$ReviewBy); ?>                  </td>
                </tr>
                <tr>
                  <td valign="top" nowrap abbr=""class="form-hdr">Review Priority: </td>
                  <td class="form-data"><? DBField("Articles","Priority",$Priority,0,$ModifyAll); ?></td>
                </tr>
                <tr>
                  <td valign="top" nowrap abbr=""class="form-hdr">Last Reviewed:</td>
                  <td class="form-data"><? if ($LastReviewed) { echo substr($LastReviewed,0,10); if ($LastReviewedBy) echo " by $LastReviewedBy"; } else echo "(never)"; ?>
                    <?					
					
					// Set button label
					if ($STATUS == "" || $STATUS == "Compose") {
					...
					?>
                    <input <? echo $R_Enabled ?> type="submit" name="Reviewed" value="<? echo $lbl ?>">                  </td>
                </tr>
                <tr>
                  <td class="form-hdr">Expires:</td>
                  <td class="form-data"><? DBField("Articles","Expires",$Expires); ?>                  </td>
                </tr>
				  </table>
				  </fieldset>				  </td>
				  </tr>
                ...
                <tr>
				  ...
				  <fieldset style="padding: 5px;"><legend>Attachments:</legend>
                  <? DisplayAttachments("Article",$ID,!$rdonly,0,1,"AND AsContent is NULL"); ?>
				  </fieldset>
				  </td>
                  <td valign="top" class="form-hdr"  width="50%" style="padding-left:10px; text-align:left;"> 
				  <fieldset style="padding: 5px;"><legend>Attachment as Content:</legend>
    ...
    <table height="450px" width="95%" cellspacing="0" cellpadding="0">
      <tr>
        <td align="center" width="100%"><!--<div class="shadowboxfloat">
            <div class="shadowcontent">!-->
              <table width="100%" class="tabtable" <? echo $FORM_STYLE ?> >
                <tr>
                  <td colspan="2" style="padding-top:6px" class="form-data"><? 
				  	if (!$AttachmentAsContent) {
						//DBField("Articles","Content",$Content);
						...
						if ($ID) {
							...
							$dir = "/" . FILES_FOLDER .  sprintf("KB%06d",$ID);
						}
						else $oFCKeditor->Config['AllowUploads' ] = 'false';
						...
						if ($ActiveTab != "Content") $TempLink = $oFCKeditor->BasePath . 'editor/fckblank.html';
						$oFCKeditor->Create($TempLink);	
						?>

<script language="javascript">
var eloc = '<? echo $oFCKeditor->Link; ?>';
...
function ShowContent()
{
	if (eloc) {
		if (document.all) {
			Content___Frame.location = eloc; // ie
		}
		else {
			var cf = FindElement("Content___Frame");
			cf.src = eloc; //firefox
		}
	}
	eloc = '';
					...
					?>
					<div style="height:470px; text-align:center; vertical-align:middle; padding-top:200px">
					<div class="MsgBox" style="width:75%;" ><img align="left" src="images/warning.gif"><br>
					The uploaded file '<? echo $AttR->Filename ?>' will be displayed as the content of this article.
					</p><p align="center" class="form-data style1"><br>
					  (
   ...
   if ($ID) { 
    ...
    <table width="95%" cellspacing="0" cellpadding="0">
      <tr>
        <td align="center" width="100%"><!--<div class="shadowboxfloat">
            <div class="shadowcontent">!-->
              <table width="100%" class="tabtable" <? echo $FORM_STYLE ?> >
			  	<tr><td class="form-hdr" style="text-align:left"><strong>Previous Article Versions:</strong></td>
			  	</tr>
                <tr> 
                  <td colspan="2" class="form-data">
				    <div style="overflow:auto; height:470px">
				  	<table cellpadding="4" cellspacing="1">
					<tr bgcolor="#CCCCCC"><td class="list-sm-hdr" >ID</td>
    ...
    <table width="95%" cellspacing="0" cellpadding="0">
      <tr>
        <td align="center" width="100%">
              <table width="100%" class="tabtable" <? echo $FORM_STYLE ?> >
			  	<tr><td class="form-hdr" style="text-align:left"><strong>Audit Trail:</strong> (most recent shown first)</td>
			  	</tr>
                <tr> 
                  <td colspan="2" class="form-data">
				    <div style="overflow:auto; height:470px">
				  	<table cellpadding="3" cellspacing="1" width:100%>
					<tr><td  class="list-sm-hdr">Date</td><td  align=center class="list-sm-hdr">Author</td><td  class="list-sm-hdr">Details</td></tr>
					<?
